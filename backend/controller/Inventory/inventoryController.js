...
// Reduce Quantity from an Existing Item
exports.reduceQuantityFromItem = async (req, res) => {
  const { itemId } = req.params;
  const { quantityToReduce, responsiblePerson, reasonForAction } = req.body;

  if (quantityToReduce === undefined || !responsiblePerson || !reasonForAction) {
    return res.status(400).json({ message: 'Missing required fields' });
  }
  ...
  try {
    ...
    res.status(400).json({ message: 'Error reducing quantity', error: error.message });
  }
};
